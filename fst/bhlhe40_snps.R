...
path_to_snp <- "C:/Users/miles_000/Downloads/Research/Tooth/results/bhlhe40_snps.fst"

# Load the data
snp_fst_data <- read.table(path_to_snp, sep="\t", header=FALSE)
# Rename the columns
names(snp_fst_data) <- c("LG", "POS", "FST")
snp_fst_data <- snp_fst_data[as.numeric(as.character(snp_fst_data$POS)) > 7127934,]
snp_fst_data <- snp_fst_data[as.numeric(as.character(snp_fst_data$POS)) < 7650068,]
...
# Plot the data
png_name <- "BHLHE40_Bicuspid_vs_Tricuspid_Fst_SNP.png"
Cairo(png_name, type="png", , width = 12, height = 4, units = 'in', res=300)
par(fig=c(0,1,0,0.8))
plot(snp_fst_data$POS, snp_fst_data$FST, col=snp_fst_data$COL, xlab="Position on LG5", ylab=expression("Fst"), xaxt="n", main="", pch=20)
...
title(main = expression("BHLHE40 Bicuspid vs Tricuspid Fst - SNPs"), line = -0.5)
